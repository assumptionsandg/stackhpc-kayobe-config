...
# Additonal pulp configuration script
# ./deploy-aufn.sh skip-local-pulp 
# to skip local pulp repo generation.
...
if [[ $1 == "skip-local-pulp" ]]
then
    echo "stackhpc_repo_mirror_url: \"http://pulp-server.internal.sms-cloud:8080\"" >> $KAYOBE_CONFIG_PATH/environments/$KAYOBE_ENVIRONMENT/stackhpc.yml
    echo "stackhpc_docker_registry: \"{{ stackhpc_repo_mirror_url | regex_replace('^https?://', '') }}\"" >> $KAYOBE_CONFIG_PATH/environments/$KAYOBE_ENVIRONMENT/stackhpc.yml
